...
from oci.config import (from_file, get_config_value_or_default,
                        DEFAULT_LOCATION, DEFAULT_PROFILE)
...
class Utilities:
    ...
    @staticmethod
    def create_signer(auth_type: str, profile=None) -> (dict, Signer):
        log.info('Creating Signer')
        log.debug(f'Signer Type: {auth_type}, {profile}')
        
        signer_func_type = {
            "instance_principal": _instance_principal,
            "delegation_token": _delegation_token,
            "profile": _profile
        }

        signer_func = signer_func_type[auth_type]
        cfg, signer = signer_func(profile)

        return cfg, signer
        
        
# Args accepted but not used to maintain conformity on signer funcs
def _instance_principal(*args) -> (dict, Signer):
    try:
...
# Args accepted but not used to maintain conformity on signer funcs
def _delegation_token(*args) -> (dict, Signer):
    try:
...
def _profile(profile: str | None) -> (dict, Signer):
    config = from_file(
        DEFAULT_LOCATION,
